extends Node
class_name LevelManager #NOTE DELETE
...
#
#func spawn_players() : #NOTE DELETE
	...
	#for i in num_players :
		#GameManager.player(i+1).respawn(p.get(i).position)
		#if randomize_spawn_rotation : GameManager.player(i+1).tank_rigidbody.rotation = randf_range(0,360)
		#else : GameManager.player(i+1).tank_rigidbody.rotation = p.get(i).rotation
#
#func tank_died() : # NOTE DELETE
	...
	#var tanks_alive : int = GameManager.alive_players_count()
	#if tanks_alive == 1 :
		#var timer = get_tree().create_timer(2.5)
		#await timer.timeout
		#if GameManager.alive_players_count() == 0 : return #if the last surviving player died while the timer was running, let the new timer that was created when it died run out before ending the scene
		#GameManager.end_of_round()
	#if tanks_alive == 0 :
		#var timer = get_tree().create_timer(2.5)
		#await timer.timeout
		#GameManager.end_of_round()
